...
function exec() {
  ...
  let studenttransactions = Banana.document.table("Transactions");
  ...
  for ( let i = 0; i < studenttransactions.rowCount; i++ ) {
    if ( studenttransactions.row(i).value("Doc") === "" && ( studenttransactions.row(i).value("AccountDebit") != "" || studenttransactions.row(i).value("AccountCredit") != "" )) {
      Banana.application.addMessage("Il documento iniziale non ha tutti i numeri di esercizio. I numeri di esercizio devono corrispondere.");
      return;
    }
  }
  ...
  let teachertransactions = teacherfile.table('Transactions');
  ...
  for ( let i = 0; i < teachertransactions.rowCount; i++ ) {
    if ( teachertransactions.row(i).value("Doc") === "" && ( teachertransactions.row(i).value("AccountDebit") != "" || teachertransactions.row(i).value("AccountCredit") != "" )) {
      Banana.application.addMessage("Il documento da importare non ha tutti i numeri di esercizio. I numeri di esercizio devono corrispondere.");
      return;
    }
  }

  jsonDoc = inserttransactions(teachertransactions, studenttransactions);
...
function inserttransactions(teachertransactions, studenttransactions) {
