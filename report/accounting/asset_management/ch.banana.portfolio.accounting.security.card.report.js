...
function exec(inData, options) {
    ...
    let dlgTitle = "Security Isin";
    let dlgLabel = "Enter the Isin number of the security";
    let scriptId = "ch.banana.portfolio.accounting.security.card.report.js";
    let docInfo = "";
    let itemsData = "";
    let itemTrans = "";
    let itemAccount = "";
    ...
    let journalData = "";
    let trIdList = ""; // transactions id List
    ...
    journalData = getTransactionsTableData(banDoc);
    //get the item related transactions array
    Banana.Ui.showText(JSON.stringify(transData));
    itemTrans = getItemRelatedTransactions(selectedItem, transData);
...
/**
 * Calculate the average accouting cost of the item 
 * @param {*} trRowBalance the actual balance
 * @param {*} trRowQtBalance the actual qt balance
 * @returns 
 */
function getCurrentAccAvgCost(trRowBalance, trRowQtBalance) {
    let context = { 'decimals': 2, 'mode': Banana.SDecimal.HALF_EVEN };

    return accAvgCost = Banana.SDecimal.divide(trRowBalance, trRowQtBalance, context);
}
...
var itemCardData = {
    ...
    "item": "IT0005239360",
    "data": [{
            "transactionType": "Purchase", //acquisto,vendita, assestamento.
            "date": "",
            "Description": "",
            "debit (baseCurr)": "",
            "credit (baseCurr)": "",
