...
var DlgInvestmentsAccounts = class DlgInvestmentsAccounts {
    constructor(banDoc) {
        this.banDoc = banDoc;
    }
    ...
    initDialogParams() {
        ...
        dialogParam.balanceAccounts.investmentsAccount = getInvestmentsAccountsFormatted(this.banDoc);
    ...
    verifyParams(userParam) {
        if (!userParam || Object.keys(userParam).length === 0) {
            userParam = this.initDialogParams();
        }
        return userParam;
    }
            ...
            if (["balanceaccounts", "valuechangingaccounts", "profitandlossaccounts"].includes(item.parentObject)) {
                if (item.value.indexOf(";") > 0) { // Could happen with "balanceaccounts".
                    ...
                    for (var i = 0; i < accounts.length; i++) {
                        if (!this.accountExists(accountsTable, accounts[i]))
                            paramsOk = false;
                    }
                } else {
                    if (!this.accountExists(accountsTable, item.value)) {
    ...
    accountExists(accountsTable, account) {
...
function exec() {
    ...
    let userParam = dlgInvestmentsAccounts.initDialogParams();
    let settingsId = "ch.banana.portfolio.accounting.accounts.dialog";

    userParam = getFormattedSavedParams(banDoc, settingsId);
    userParam = dlgInvestmentsAccounts.verifyParams(userParam);
    let dlgTitle = 'Account Settings';
    let convertedParam = dlgInvestmentsAccounts.convertParam(userParam);
