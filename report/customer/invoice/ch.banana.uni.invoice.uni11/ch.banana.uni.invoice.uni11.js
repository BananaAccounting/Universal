...
// @description.de = [UNI11] Programmierbares Rechnungs-Layout
// @description.fr = [UNI11] Layout de facture programmable
// @description.en = [UNI11] Programmable Invoice layout
// @doctype = *
...
/*
  ...
  Same code of the 'CH10' extension, but without the Swiss QRCode part.
  1) Copy and paste all the "ch.banana.ch.invoice.ch10.js" code.
  2) Remove all the QRCode references:
    - "convertParamQR(convertedParam, userParam, texts)" function call.
    - "initParamQR(userParam)" function call.
    - "verifyParamQR(userParam)" function call.
    - "printQRCode(invoiceObj, repDocObj, repStyleObj, userParam)" function call.
    - "setQrSettingsTexts(language, texts)" function call.
*/
...
function getUserColumnValue(banDoc, originRow, itemNumber, column) {
  ...
  */
  ...
  if (IS_INTEGRATED_INVOICE) {
    ...
    else if (column.startsWith("I.") && banDoc.table('Items')) {
...
function bananaRequiredVersion(requiredVersion, expmVersion) {
  ...
  if (Banana.compareVersion && Banana.compareVersion(Banana.application.version, requiredVersion) < 0) {
    ...
    switch(language) {
      ...
      case "nl":
        if (expmVersion) {
          msg = "Het script werkt niet met deze versie van Banana Accounting. Upgrade naar Banana+ Dev Channel (" + requiredVersion + ").";
        } else {
      ...
      case "zh":
        if (expmVersion) {
          msg = "脚本无法在此版本的Banana财务会计软件中运行。请更新至 下载 Banana+ 的开发频道 (" + requiredVersion + ").";
        } else {
      ...
      case "es":
        if (expmVersion) {
          msg = "Este script no se ejecuta con esta versión de Banana Accounting. Por favor, actualice a Banana+ Dev Channel (" + requiredVersion + ").";
        } else {
      ...
      case "pt":
        if (expmVersion) {
          msg = "Este script não é executado com esta versão do Banana Accounting. Por favor, atualize para Banana+ Dev Channel (" + requiredVersion + ").";
        } else {
